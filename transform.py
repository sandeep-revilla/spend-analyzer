...
def _detect_is_deleted_mask(df: pd.DataFrame) -> Optional[pd.Series]:
    ...
    values ('true','t','1','yes','y','false','0','no', etc.). Returns None if no such column exists.
    """
    ...
    # Coerce to string safely for mixed types and then test tokens
    try:
        # boolean dtype
        if pd.api.types.is_bool_dtype(s):
            return s.fillna(False).astype(bool)
        # numeric dtype: treat non-zero as deleted
        if pd.api.types.is_numeric_dtype(s):
            return s.fillna(0).astype(int) != 0
    except Exception:
        pass

    # Fallback: string-based detection (handles 'TRUE','False','1','0','yes','no', etc.)
    lowered = s.astype(str).str.strip().str.lower().fillna("")
    true_tokens = {"true", "t", "1", "yes", "y"}
    false_tokens = {"false", "f", "0", "no", "n", ""}
    # treat anything in true_tokens as deleted; anything else -> not deleted
    mask = lowered.isin(true_tokens)
    # ensure returned Series aligned with df index
    mask.index = df.index
    return mask
...
def _prefer_datetime_column(df: pd.DataFrame) -> Optional[str]:
    ...
    for c in df.columns:
        lc = str(c).lower()
        if lc in ("datetime", "date_time", "date-time", "timestamp"):
            return c
    for c in df.columns:
...
def _coerce_amount_series(series: pd.Series) -> pd.Series:
    """
    Convert a Series to numeric amounts:
      - strip currency symbols, commas, parentheses
      - handle negative in parentheses like "(1,234.50)"
      - return float Series with NaN for non-parseable entries
    """
    if series is None:
...
def convert_columns_and_derives(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 2) If not found, attempt heuristic detection over columns
    ...
    # 3) fallback: try to find any object column parseable as datetime
    if primary_dt_col is None:
        for col in df.columns:
            try:
    ...
    # 4) Detect amount-like columns and coerce to numeric
    amount_cols = []
    for col in df.columns:
        if _is_amount_like_column(col):
            try:
                coerced = _coerce_amount_series(df[col])
                df[col] = coerced
                amount_cols.append(col)
            except Exception:
                continue

    # Heuristic: if no obvious amount column found, try to find object column that looks numeric
    if not amount_cols:
        for col in df.columns:
            try:
                if df[col].dtype == object:
                    sample = df[col].astype(str).head(30).str.replace(r"[^\d\.\-]", "", regex=True)
                    parsed = pd.to_numeric(sample.replace("", pd.NA), errors="coerce")
                    if parsed.notna().sum() >= 3:
                        df[col] = _coerce_amount_series(df[col])
                        amount_cols.append(col)
                        break
            except Exception:
                continue
    ...
    for candidate in candidates:
        if candidate in lower_cols:
            preferred = lower_cols[candidate]
            break
    ...
    if preferred:
        if preferred != "Amount":
            if "Amount" not in df.columns:
                try:
                    df = df.rename(columns={preferred: "Amount"})
                except Exception:
    ...
    # 5) Create canonical timestamp and date columns (prefer primary_dt_col)
    try:
        if primary_dt_col:
    ...
    # 6) Infer or normalize Type (case-insensitive detection of existing 'type' column)
    ...
    try:
        ...
        df["Type"] = df["Type"].replace(
            {"nan": "unknown", "none": "unknown", "": "unknown", "na": "unknown", "null": "unknown"}
        )
    except Exception:
        df["Type"] = "unknown"
...
def compute_daily_totals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Compute daily totals DataFrame with columns:
      ...
      - Total_Spent (float)  <- sum of debits / spend (positive)
      - Total_Credit (float) <- sum of credits (positive)

    Logic:
      - If 'Type' column exists and contains debit/credit: use it.
      - Else: treat positive Amount as spend (debit), negative as credit.
      - Ensure Total_Credit is returned as positive numbers for display.
    """
    ...
    if "Type" in w.columns and w["Type"].astype(str).str.strip().any():
        ...
        if not debit_df.empty:
            daily_spend = (
                debit_df.groupby(debit_df["_group_date"])["Amount_numeric"]
                .sum()
                .reset_index()
                .rename(columns={"_group_date": "Date", "Amount_numeric": "Total_Spent"})
            )

        if not credit_df.empty:
            daily_credit = (
                credit_df.groupby(credit_df["_group_date"])["Amount_numeric"]
                .sum()
                .reset_index()
                .rename(columns={"_group_date": "Date", "Amount_numeric": "Total_Credit"})
            )
            # if credits are stored negative, make them positive for reporting
            if (daily_credit["Total_Credit"] < 0).any():
